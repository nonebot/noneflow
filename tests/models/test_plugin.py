...
def test_plugin_info_validation_failed(mocker: MockerFixture) -> None:
    ...
    assert (
        e.value.message
        == """> Plugin: name\n\n**⚠️ 在发布检查过程中，我们发现以下问题:**\n<pre><code><li>⚠️ 包 <a href="https://pypi.org/project/project_link_failed/">project_link_failed</a> 未发布至 PyPI。<dt>请将您的包发布至 PyPI。</dt></li><li>⚠️ 项目 <a href="https://www.baidu.com">主页</a> 返回状态码 404。<dt>请确保您的项目主页可访问。</dt></li><li>第 2 个标签名称不能超过 10 个字符</li><li>第 2 个标签颜色不符合十六进制颜色码规则</li></code></pre>"""
    )
    calls = [
